...
class CheckoutView(EcomMixin,CreateView):
    ...
    def form_valid(self, form):
        ...
        if cart_id:
            ...
            order = form.save()
            if pm == 'khalti':
                return redirect(reverse('store:khaltirequest') + "?o_id=" + str(order.id))
        else:
    ...
    def dispatch(self, request, *args, **kwargs):
        if request.user.is_authenticated and Customer.objects.filter(user=request.user).exists():
            ...
            if request.user.customer != order.cart.customer:
             return redirect('store:customerprofile')
        else:
            return redirect('/login/?next=/checkout/')
        return super().dispatch(request, *args, **kwargs)
...
class ForgotPasswordView(TemplateView):
    ...
    def form_valid(self):
        ...
        text_content = "We heard that you lost your GitHub password. \nSorry about that! But don\'t worry! You can use the following link to reset your password:"
...
class PasswordResetView(FormView):
    template_view='passwordreset.html'
...
#Admin pages (for code complexity here we inheritting customer form in form class)
...
class AdminProductCreateView(AdminRequiredMixin, CreateView):
    ...
    success_url = reverse_lazy('store:adminproductcreate')
    ...
    def form_valid(self, form):
        ...
        for i in images:
            Product.objects.create(product=p, image=i)
        return super().form_valid(form)
...
def contact(request):
    if request.method == 'POST':
        ...
        return render(request,'samplecontactform.html',{'message_name': message_name})
    else:
        return render(request,'samplecontactform.html', {})

class ContactView(EcomMixin,TemplateView):
    template_name = 'contact.html'

class PrivacyPolicyView(EcomMixin,TemplateView):
    template_name = 'privacypolicy.html'

class RetexcView(EcomMixin,TemplateView):
    template_name = 'retexc.html'

class ShippingPolicyView(EcomMixin,TemplateView):
    template_name = 'shippingpolicy.html'

class TermsandConditionsView(EcomMixin,TemplateView):
